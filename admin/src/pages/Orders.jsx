...
const Orders = ({token}) => {
  console.log("Token received in Orders component:", token);

const [orders,setOrders]=useState([])
const fetchAllOrders=async ()=>{
  console.log("Fetching all orders..."); // Debugging log
if(!token){
  console.log("No token found, skipping request"); // Debugging log
  return null;
}
try {
...
const statusHandler =async (event,orderId)=>{
  ...
  try {
    const response = await axios.post(
      ...
      { orderId, status: event.target.value }, // Corrected request body
      { headers: { token } } // Moved headers to the third argument
    );
